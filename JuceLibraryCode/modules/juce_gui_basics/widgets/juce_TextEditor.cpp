...
void TextEditor::textChanged()
{
    ...
    if (listeners.size() != 0 || onTextChange != nullptr || onTextChangeDelegates.size() != 0)
...
{
    //allow scrolling the parent even when not focused
    if (!isMouseOver())
        return;
...
bool TextEditor::keyPressed (const KeyPress& key)
{
    ...
    for (auto& keyNotConsumed : keyPressesNotConsumed)
    {
        if (keyNotConsumed == key)
            return false;
    }
    ...
    return consumeNoEffectKeyPresses;
}
...
void TextEditor::handleCommandMessage (const int commandId)
{
    ...
    case TextEditorDefs::textChangeMessageId:
        ...
        if (!checker.shouldBailOut())
        {
            ...
            for (auto& delegate : onTextChangeDelegates)
            {
                if (delegate)
                    delegate();
            }
